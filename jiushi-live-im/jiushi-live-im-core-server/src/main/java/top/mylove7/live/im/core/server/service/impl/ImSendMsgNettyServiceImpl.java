...
@Service
@Slf4j
public class ImSendMsgNettyServiceImpl implements IImSendMsgNettyService {
    ...
    private boolean checkOrAddImMsgRecord(ImMsgBodyInTcpWsDto imMsg) {
        ...
        Assert.notNull(imMsg.getMsgId(),"消息id为空非法请求");
        ...
        if (imAckDto != null && imAckDto.getRetryTime() > 3 ) {
            log.info("【{}】该消息达到最大发送次数",imMsg.getMsgId());
            return false;
        }
        if (imAckDto != null && imAckDto.getHadAck()) {
            log.info("【{}】该消息已发送并确认到达",imMsg.getMsgId());
            return false;
        }
        if (imAckDto == null) {
